...
Per-provider setup instructions — with field-by-field tables for OpenAI-compatible
services (OpenAI, Ollama, vLLM, LM Studio, LiteLLM, OpenRouter), Anthropic, Azure OpenAI,
Claude in Azure AI Foundry, Google Gemini, and AWS Bedrock's three authentication
methods — are in the [Provider Setup Guide](PROVIDERS.md).

Provider API keys can be entered directly or referenced through an environment variable
set on the gateway host (preferred for production). Stored secrets are write-only in the
admin UI: they are never returned to the browser, and saving a provider with a blank
